//***************************************************************************
// 
// 파일: soft_renderer_3d.cpp
// 
// 설명: DirectDraw7을 활용한 3D 소프트웨어 렌더러
// 
// 작성자: bumpsgoodman
// 
// 작성일: 2023/11/09
...
typedef struct soft_renderer_3d
{
    i_soft_renderer_3d_t base;
    size_t ref_count;
...
extern "C" void __stdcall create_vertex_buffer3_private(i_vertex_buffer3_t** pp_out_vertex_buffer);
extern "C" void __stdcall create_index_buffer_private(i_index_buffer_t** pp_out_index_buffer);
extern "C" void __stdcall create_mesh3_private(i_mesh3_t** pp_out_mesh);

static bool __stdcall create_back_buffer_private(soft_renderer_3d_t* p_renderer, const size_t width, const size_t height);

static vector3_t __stdcall to_screen_coord(const vector3_t cartesian_coord, const size_t screen_width, const size_t screen_height);
static vector3_t __stdcall to_cartesian_coord(const vector3_t screen_coord, const size_t screen_width, const size_t screen_height);
...
static size_t __stdcall add_ref(i_soft_renderer_3d_t* p_this)
{
    ASSERT(p_this != NULL, "p_this == NULL");

    soft_renderer_3d_t* p_renderer = (soft_renderer_3d_t*)p_this;
    return ++p_renderer->ref_count;
}

static size_t __stdcall release(i_soft_renderer_3d_t* p_this)
{
    ASSERT(p_this != NULL, "p_this == NULL");
    soft_renderer_3d_t* p_renderer = (soft_renderer_3d_t*)p_this;
...
static size_t __stdcall get_ref_count(const i_soft_renderer_3d_t* p_this)
{
    ASSERT(p_this != NULL, "p_this == NULL");

    const soft_renderer_3d_t* p_renderer = (soft_renderer_3d_t*)p_this;
    return p_renderer->ref_count;
}

static void __stdcall update_window_pos(i_soft_renderer_3d_t* p_this)
{
    ASSERT(p_this != NULL, "p_this == NULL");
...
static void __stdcall update_window_size(i_soft_renderer_3d_t* p_this)
{
    ASSERT(p_this != NULL, "p_this == NULL");
...
static bool __stdcall initialize(i_soft_renderer_3d_t* p_this, HWND hwnd)
{
    ASSERT(p_this != NULL, "p_this == NULL");
...
failed_init:
    release(p_this);
    memset(p_renderer, 0, sizeof(soft_renderer_3d_t));
    return false;
}

static size_t __stdcall get_width(const i_soft_renderer_3d_t* p_this)
{
    ASSERT(p_this != NULL, "p_this == NULL");
...
static size_t __stdcall get_height(const i_soft_renderer_3d_t* p_this)
{
    ASSERT(p_this != NULL, "p_this == NULL");
...
static bool __stdcall begin_draw(i_soft_renderer_3d_t* p_this)
{
    ASSERT(p_this != NULL, "p_this == NULL");
...
static void __stdcall end_draw(i_soft_renderer_3d_t* p_this)
{
    ASSERT(p_this != NULL, "p_this == NULL");
...
static void __stdcall on_draw(const i_soft_renderer_3d_t* p_this)
{
    ASSERT(p_this != NULL, "p_this == NULL");
...
static bool __stdcall begin_gdi(i_soft_renderer_3d_t* p_this, HDC* p_out_hdc)
{
    ASSERT(p_this != NULL, "p_this == NULL");
...
static void __stdcall end_gdi(i_soft_renderer_3d_t* p_this, const HDC hdc)
{
    ASSERT(p_this != NULL, "p_this == NULL");
...
static void __stdcall print_text(i_soft_renderer_3d_t* p_this, const HDC hdc,
...
static void __stdcall clear(i_soft_renderer_3d_t* p_this, const uint32_t rgb)
{
    ASSERT(p_this != NULL, "p_this == NULL");
...
static void __stdcall draw_pixel(i_soft_renderer_3d_t* p_this, const int dx, const int dy, const uint32_t rgb)
{
    ASSERT(p_this != NULL, "p_this == NULL");
...
static void __stdcall draw_rectangle(i_soft_renderer_3d_t* p_this, const int dx, const int dy, const size_t width, const size_t height, const uint32_t rgb)
{
    ASSERT(p_this != NULL, "p_this == NULL");
...
static void __stdcall draw_line(i_soft_renderer_3d_t* p_this, const int sx, const int sy, const int dx, const int dy, const uint32_t rgb)
{
    ASSERT(p_this != NULL, "p_this == NULL");
...
static void __stdcall draw_bitmap(i_soft_renderer_3d_t* p_this, const int dx, const int dy, const int sx, const int sy, const size_t sw, const size_t sh, const size_t width, const size_t height, const char* p_bitmap)
{
    ASSERT(p_this != NULL, "p_this == NULL");
...
static bool __stdcall create_vertex_buffer(const i_soft_renderer_3d_t* p_this,
                                           ...
                                           i_vertex_buffer3_t** pp_out_vertex_buffer)
{
    ASSERT(p_this != NULL, "p_this == NULL");
    ...
    i_vertex_buffer3_t* p_vertex_buffer;
    create_vertex_buffer3_private(&p_vertex_buffer);

    vertex_buffer3_t* p_buffer = (vertex_buffer3_t*)p_vertex_buffer;
    ...
    for (size_t i = 0; i < num_semantics; ++i)
    {
        ...
        case SEMANTIC_TEXCOORD:
            p_buffer->pa_texcoords = (vector3_t*)malloc(sizeof(vector3_t) * num_vertices);
    ...
    *pp_out_vertex_buffer = (i_vertex_buffer3_t*)p_buffer;
...
static bool __stdcall create_index_buffer(const i_soft_renderer_3d_t* p_this,
...
static bool __stdcall create_mesh(const i_soft_renderer_3d_t* p_this,
                                  i_vertex_buffer3_t* p_vertex_buffer,
                                  i_index_buffer_t* p_index_buffer,
                                  i_texture_t* p_texture_or_null,
                                  const color_t wireframe_color,
                                  i_mesh3_t** pp_out_mesh)
{
    ASSERT(p_this != NULL, "p_this == NULL");
    ...
    mesh3_t* p_mesh;
    create_mesh3_private((i_mesh3_t**)&p_mesh);
...
static void __stdcall draw_mesh(i_soft_renderer_3d_t* p_this, const i_mesh3_t* p_mesh, const matrix_t* p_transform_mat, const bool b_wireframe)
{
    ASSERT(p_this != NULL, "p_this == NULL");
    ...
    const vertex_buffer3_t* p_vertex_buffer = (vertex_buffer3_t*)p_mesh->vtbl->get_vertex_buffer(p_mesh);

    const vector3_t* p_positions = p_vertex_buffer->pa_positions;
    const color_t* p_colors = p_vertex_buffer->pa_colors;
    const vector3_t* p_texcoords = p_vertex_buffer->pa_texcoords;
    ...
    const size_t num_triangles = p_index_buffer->num_indices;
    for (size_t j = 0; j < num_triangles; ++j)
    {
        ...
        if (b_wireframe)
        {
            const uint32_t color = color_to_argb(p_mesh->vtbl->get_wireframe_color(p_mesh));
        ...
        else
        {
            ...
            for (int y = (int)min.y; y <= (int)max.y; ++y)
            {
                for (int x = (int)min.x; x <= (int)max.x; ++x)
                {
                    ...
                    if (b_texcoords)
                    {
                        const vector3_t target_uv = { uv0.x * w0 + uv1.x * w1 + uv2.x * w2, uv0.y * w0 + uv1.y * w1 + uv2.y * w2 };
...
void __stdcall create_instance(void** pp_out_instance)
{
    ...
    static i_soft_renderer_3d_vtbl_t vtbl =
    {
        add_ref,
        release,
        get_ref_count,
    ...
    soft_renderer_3d_t* pa_renderer = (soft_renderer_3d_t*)malloc(sizeof(soft_renderer_3d_t));
    if (pa_renderer == NULL)
    {
